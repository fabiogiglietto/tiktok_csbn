...
generate_network_visualization <- function(coordination_results,
  ...
  tryCatch({
    ...
    # Calculate component membership and set colors
    comp <- igraph::components(graph)$membership
    data$nodes$group <- comp
    ...
    if(exists("cluster_labels") && is.data.frame(get("cluster_labels")) && nrow(get("cluster_labels")) > 0) {
      ...
      data$nodes$group <- label_lookup$group_label[match(comp, label_lookup$component_id)]
    } else {
      data$nodes$group <- paste("Network", comp)
    }
    ...
    network <- visNetwork::visNetwork(
      ...
      main = list(
        ...
        style = "font-family: 'Comfortaa', sans-serif; font-size: 16px; font-weight: bold;"
      )
    ) %>%
      ...
      visNetwork::visOptions(
        ...
        selectedBy = list(
          variable = "group",
          main = "Select Network",
          style = "width: 200px; height: 30px; margin: 8px; border: 1px solid #ddd; border-radius: 4px;"
        ),
