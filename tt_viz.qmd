...
# Modify save_output to use daily directories
update_account_logs <- function(coordination_results) {
  today_date <- format(Sys.Date(), "%Y-%m-%d")
  
  # Define file paths using daily directory
  log_file_path <- file.path(daily_output_dir, "new_accounts_log.csv")
  accounts_file_path <- file.path(daily_output_dir, "tiktok_coordinated_accounts_ids.csv")
...
generate_summary <- function(
  ...
  # Add warning if errors were encountered
  if (length(global_error_list) > 0) {
    summary_parts$warning <- sprintf(
...
generate_error_summary <- function(global_error_list) {
  ...
  if (length(global_error_list) > 0) {
    ...
    error_parts <- c(
      ...
      '\n',  # Important blank line after summary
    ...
    # Combine warning box and error details
...
# In the generate_summary function, modify the results printing part:
if (length(global_error_list) > 0) {
    summary_parts$warning <- sprintf(
      "\n\n%s",
      generate_error_summary(global_error_list)
    )
}

# In the generate_summary function, modify the results printing part to:
# Change this part in your generate_summary function
if (length(global_error_list) > 0) {
    summary_parts$warning <- sprintf(
      "\n\n%s",
      generate_error_summary(global_error_list)
...
```{r display_previous_summaries, include=TRUE, echo=FALSE, results='asis'}
# Display last 5 days of summaries
last_5_dates <- tail(sort(names(historical_data$summaries), decreasing = TRUE), 5)

for (date in last_5_dates) {
  cat("\n## Summary for", date, "\n")
  cat(paste(historical_data$summaries[[date]], collapse = "\n"))
  cat("\n")
}
```
...
generate_network_visualization <- function(coordination_results) {
  ...
  tryCatch({
    ...
    # Create base nodes data frame with component info
    data$nodes <- data$nodes %>%
      mutate(
        size = degree(graph) * 3 + 10,
        component_id = as.numeric(components(graph)$membership)
      )
    
    # Add labels if available
    if(exists("cluster_labels") && is.data.frame(cluster_labels) && nrow(cluster_labels) > 0) {
      ...
      label_lookup <- cluster_labels %>%
        ...
        select(component_id, group_label)
      
      # Join with nodes
      data$nodes <- data$nodes %>%
        left_join(label_lookup, by = "component_id") %>%
        mutate(
          group = coalesce(group_label, sprintf("Network %d", component_id))
        )
    } else {
      data$nodes <- data$nodes %>%
        mutate(
          group = sprintf("Network %d", component_id)
        )
    }
    ...
    # Create custom CSS for the dropdown
    dropdown_css <- paste0(
      "<style>",
      ...
      "  color: #212529;",
      "}",
      ".vis-network-configuration select {",
      "  font-family: inherit;",
      "  font-size: inherit;",
      "  color: inherit;",
...
# Execute network visualization if we have coordination results
if (exists("coordination_results") && !is.null(coordination_results$coord_graph)) {
  log_info("Starting network visualization generation")
  network_viz <- generate_network_visualization(coordination_results)
} else {
...
```{r interactive-plot, include=TRUE, echo=FALSE, fig.cap="Interactive visualization of coordinated account detected today. Click on a node to visit the respective TikTok account."}

# Check if network visualization is available and display it
...
```{r setup_tables, include=FALSE, echo=FALSE, message=FALSE, eval=TRUE}

# Helper function to create TikTok profile URL
create_tiktok_url <- function(username) {
  sprintf('<a href="https://tiktok.com/@%s" target="_blank">@%s</a>', username, username)
}

# 1. Coordinated Accounts Table
create_accounts_table <- function(summary_accounts, coord_graph, all_videos) {
  # Calculate network metrics for each account
  degree_scores <- degree(coord_graph)
  strength_scores <- strength(coord_graph)
  betweenness_scores <- betweenness(coord_graph, normalized = TRUE)
  
  network_metrics <- data.frame(
    account_id = names(degree_scores),
    degree = as.numeric(degree_scores),
    strength = as.numeric(strength_scores),
    betweenness = as.numeric(betweenness_scores)
  )
  
  # Calculate average engagement metrics per account
  ...
  components_df <- data.frame(
    account_id = V(coord_graph)$name,
    component = components(coord_graph)$membership
  )
  
  accounts_table <- summary_accounts %>%
    mutate(
      account_link = sapply(account_id, create_tiktok_url)
    ) %>%
    left_join(network_metrics, by = "account_id") %>%
    left_join(components_df, by = "account_id") %>%
    left_join(engagement_metrics, by = c("account_id" = "author_name")) %>%
    mutate(
      avg_time_delta = round(avg_time_delta, 2),
      avg_edge_symmetry_score = round(avg_edge_symmetry_score, 3),
    ...
    datatable(
      ...
      options = list(
        ...
        initComplete = JS(
          ...
          "$(this.api().table().container()).css({'font-size': '12px'});", # Increased font size
          "}"
        ),
        columnDefs = list(
          list(className = 'dt-right', targets = 1:11), 
          list(width = '200px', targets = 0),  # Adjust width of Account column
          list(targets = 0, render = JS(   # Truncate Account names and show full name on hover
...
# 2. Coordinated Networks/Clusters Table
create_clusters_table <- function(coord_graph, summary_accounts, all_videos) {
  ...
  clusters <- data.frame(
    component = 1:comp$no,
    size = tabulate(comp$membership),
    density = sapply(1:comp$no, function(i) {
      subg <- induced_subgraph(coord_graph, which(comp$membership == i))
      edge_density(subg)
    }),
    diameter = sapply(1:comp$no, function(i) {
      subg <- induced_subgraph(coord_graph, which(comp$membership == i))
      if(vcount(subg) < 2) return(0)
      diameter(subg)
    }),
    transitivity = sapply(1:comp$no, function(i) {
      subg <- induced_subgraph(coord_graph, which(comp$membership == i))
      transitivity(subg, type = "global")
    })
  )
  
  # Calculate average engagement metrics per component
  engagement_metrics <- all_videos %>%
    left_join(
      data.frame(
  ...
  # Prepare base clusters data frame
  clusters <- clusters %>%
    mutate(
      accounts = sapply(1:comp$no, function(i) {
        paste(V(coord_graph)$name[comp$membership == i], collapse = ", ")
      })
    ) %>%
    left_join(engagement_metrics, by = c("component"))
  ...
  if(exists("cluster_labels") && is.data.frame(cluster_labels) && nrow(cluster_labels) > 0) {
    ...
    clusters$label <- NA_character_
  }
  
  # Format final table
  clusters <- clusters %>%
    mutate(
      network_id = sprintf("Network %d", component),
      network_label = ifelse(!is.na(label) & label != "", 
                           sprintf("%s: %s", network_id, label),
                           network_id)
    ) %>%
    select(
...
# 3. Coordinated Posts Table
create_posts_table <- function(summary_groups, all_videos) {
  tryCatch({
    # Helper function to create video link
    create_video_link <- function(url) {
      if (!is.na(url)) {
    ...
    # Check if join produced results
    if(nrow(posts_table) == 0) {
      log_warn("No matching posts found after join")
      return(NULL)
    }
    
    posts_table <- posts_table %>%
      mutate(
        video_link = sapply(video_url, create_video_link)
      ) %>%
      select(
        `Post URL` = video_link,
        Author = author_name,
      ...
      distinct() %>%
      mutate(
        Description = substr(Description, 1, 150),
        Description = paste0(Description, "...")
      )
    
    datatable(posts_table,
      options = list(
        pageLength = 10,
        scrollX = TRUE,
    ...
    log_error(sprintf("Error creating posts table: %s", e$message))
    return(NULL)
  })
}

# Update the main table creation section with error handling:
if (exists("coordination_results") && exists("all_videos")) {
  log_info("Starting table creation")
  
  # Create accounts table
  tryCatch({
    log_info("Creating accounts table")
    accounts_table <<- create_accounts_table(
      coordination_results$summary_accounts, 
      coordination_results$coord_graph,
      all_videos
    )
    log_info("Accounts table created successfully")
  }, error = function(e) {
    log_error(sprintf("Error creating accounts table: %s", e$message))
    accounts_table <<- NULL
  })
  
  # Create clusters table
  tryCatch({
    log_info("Creating clusters table")
    clusters_table <<- create_clusters_table(
      coordination_results$coord_graph, 
      coordination_results$summary_accounts,
      all_videos
    )
    log_info("Clusters table created successfully")
  }, error = function(e) {
    log_error(sprintf("Error creating clusters table: %s", e$message))
    clusters_table <<- NULL
  })
  
  # Create posts table
  tryCatch({
    log_info("Creating posts table")
    posts_table <<- create_posts_table(
      coordination_results$summary_groups, 
      all_videos
    )
    log_info("Posts table created successfully")
  }, error = function(e) {
    log_error(sprintf("Error creating posts table: %s", e$message))
    posts_table <<- NULL
  })
  ...
  log_warn("Missing required data for table creation")
  if (!exists("coordination_results")) log_warn("coordination_results not found")
  if (!exists("all_videos")) log_warn("all_videos not found")
}

```

## Coordinated Accounts
```{r display_accounts, echo=FALSE, warning=FALSE}

if (exists("accounts_table") && !is.null(accounts_table)) {
  accounts_table
} else {
  cat("No coordinated accounts data available.")
}
```

## Coordinated Networks/Clusters
```{r display_clusters, echo=FALSE, warning=FALSE}

if (exists("clusters_table") && !is.null(clusters_table)) {
  clusters_table
} else {
  cat("No network clusters data available.")
}

```

## Coordinated Posts
```{r display_posts, echo=FALSE, warning=FALSE}

if (exists("posts_table") && !is.null(posts_table)) {
  posts_table
} else {
  cat("No coordinated posts data available.")
}
...
prepare_download_files <- function() {
  ...
  download_dir <- file.path(daily_output_dir, "download")
  dir.create(download_dir, showWarnings = FALSE, recursive = TRUE)
  
  # Save coordinated accounts
  if (exists("coordination_results") && !is.null(coordination_results$summary_accounts)) {
    write.csv(coordination_results$summary_accounts, 
              file.path(download_dir, "coordinated_accounts.csv"), 
              row.names = FALSE)
  }
  
  # Save network clusters
  if (exists("coordination_results") && !is.null(coordination_results$coord_graph)) {
    # Get components
    comp <- components(coordination_results$coord_graph)
    clusters_data <- data.frame(
      component = 1:comp$no,
      size = tabulate(comp$membership)
    )
    write.csv(clusters_data, 
              file.path(download_dir, "network_clusters.csv"), 
              row.names = FALSE)
  }
  
  # Save coordinated posts
  if (exists("coordination_results") && !is.null(coordination_results$summary_groups)) {
    write.csv(coordination_results$summary_groups,
              file.path(download_dir, "coordinated_posts.csv"),
              row.names = FALSE)
  }
  
  # Copy network visualization if it exists
  viz_file <- file.path(daily_output_dir, "network", "coordination_network.html")
  if (file.exists(viz_file)) {
    file.copy(viz_file,
              file.path(download_dir, "network_visualization.html"),
              overwrite = TRUE)
  }
  
  # Create zip file
  current_date <- format(Sys.Date(), "%Y-%m-%d")
  zip_file <- file.path(daily_output_dir, sprintf("coordination_results_%s.zip", current_date))
  
  # Create zip file from download directory
  zip(zip_file, download_dir, flags = "-r9X")
  
  return(zip_file)
}

# Create the zip file
zip_file <- prepare_download_files()

# Create download button HTML
if (file.exists(zip_file)) {
  download_url <- paste0("coordination_results_", format(Sys.Date(), "%Y-%m-%d"), ".zip")
