...
```{r fetch_recent_videos, message=FALSE}
...
# Identify videos that have both identical descriptions and the same music_id, appearing at least twice
videos_with_same_desc_and_music <- recent_videos %>%
  group_by(video_description, music_id) %>%
  filter(n() >= 2) %>%
  ungroup()
...
unique_videos_desc_and_music <- videos_with_same_desc_and_music %>%
  select(video_description, music_id) %>%
  distinct()
```

```{r fetch_all_videos, message=FALSE}
...
for(i in 1:nrow(unique_videos_desc_and_music)) {
  tryCatch({
    
    # Extract the current video description and music_id
    desc <- unique_videos_desc_and_music$video_description[i]
    music_id <- unique_videos_desc_and_music$music_id[i]
    ...
    temp_videos <- query() |>
      query_and(field_name = "keyword",
                operation = "EQ",
                field_values = desc) |>
      query_and(field_name = "music_id",
            operation = "EQ",
            field_values = music_id) |>
      tt_search_api(start_date = startdate, 
                    end_date = enddate,
                    max_pages = 100L,
                    verbose = TRUE,
    ...
    # Append the results to the list with a unique key combining description and music ID
    key <- paste(desc, music_id, sep = " ")
    all_videos_list[[key]] <- temp_videos
...
We have analyzed `r scales::comma(nrow(recent_videos))` videos from a set of `r scales::comma(length(coord_users))` monitored accounts. Those accounts posted `r scales::comma(nrow(all_videos))` videos between `r format(as.Date(startdate), "%B %d, %Y")` and `r format(as.Date(enddate), "%B %d, %Y")`. Our coordinated detection analysis of these videos identified `r scales::comma(nrow(summary_accounts))` accounts across `r scales::comma(igraph::components(coord_graph)$no)` components and discovered `r scales::comma(nrow(new_account_ids))` new accounts exhibiting coordinated behavior.
...
```{r interactive-plot, fig.cap="Interactive visualization of coordinated account detected today. Click on a node click to visit the respective TikTok account."}
# Print the network
network
```
