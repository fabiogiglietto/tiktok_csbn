...
fetch_all_videos_smart <- function(recent_videos, start_date, end_date) {
  if (!should_continue_processing()) {
    log_warn("Skipping all videos fetch due to quota limits")
    return(data.frame())
  }
  
  # Start with yesterday's full data if available (for description matching)
  base_videos <- data.frame()
  
  if (exists("smart_reuse_result") && nrow(smart_reuse_result$reusable_videos) > 0) {
    # Load full yesterday's data for broader description matching
    yesterday_paths <- get_yesterday_data_path()
    if (file.exists(yesterday_paths$snapshot)) {
      tryCatch({
        yesterday_full_data <- readr::read_csv(yesterday_paths$snapshot, 
                                              col_types = cols(
                                                video_id = col_character(), 
                                                effect_ids = col_character(),
                                                music_id = col_character(), 
                                                hashtag_names = col_character(),
                                                create_time = col_character()  # Keep as character
                                              ))
        
        # Ensure create_time is character format for consistency
        yesterday_full_data <- yesterday_full_data %>%
          mutate(create_time = as.character(create_time))
        
        # Standardize data types
        yesterday_full_data <- standardize_data_types(yesterday_full_data, "yesterday_full")
        
        # Filter yesterday's data to current date range for all videos (not just recent_videos accounts)
        yesterday_filtered <- yesterday_full_data %>%
          filter(
            as.Date(create_time) >= as.Date(start_date),
            as.Date(create_time) <= as.Date(end_date)
          )
        
        base_videos <- yesterday_filtered
        log_info(sprintf("Starting with %d videos from yesterday's full dataset within date range", 
                        nrow(base_videos)))
      }, error = function(e) {
        log_warn(sprintf("Could not load yesterday's full data: %s", e$message))
      })
    }
  }
  ...
  if (is.null(recent_videos) || nrow(recent_videos) == 0) {
    ...
    # If we have base videos from yesterday, process and return them
    if (nrow(base_videos) > 0) {
      log_info("Using only yesterday's data since no recent videos available")
  ...
  # Get unique descriptions from recent videos for API queries
  unique_descriptions <- unique(recent_videos$video_description[nchar(recent_videos$video_description) >= 80])
  
  # Filter descriptions that might already be covered by yesterday's data
  if (nrow(base_videos) > 0) {
    existing_descriptions <- unique(base_videos$video_description)
    new_descriptions <- setdiff(unique_descriptions, existing_descriptions)
    
    log_info(sprintf("Description analysis: %d total, %d already covered, %d need fresh fetch", 
                    length(unique_descriptions), 
                    length(unique_descriptions) - length(new_descriptions),
                    length(new_descriptions)))
    
    unique_descriptions <- new_descriptions
  }
  
  log_info(sprintf("Querying %d unique descriptions via API", length(unique_descriptions)))
  
  # Process new descriptions in batches
  api_results <- data.frame()
  if (length(unique_descriptions) > 0) {
    api_results <- batch_fetch_videos(
      unique_descriptions, 
      start_date, 
      end_date,
      batch_size = 3,
    ...
    # Ensure create_time consistency in API results
    if (nrow(api_results) > 0) {
      api_results <- standardize_data_types(api_results, "api_results")
    }
  }
  
  # Standardize base_videos if it exists
  if (nrow(base_videos) > 0) {
    base_videos <- standardize_data_types(base_videos, "base_videos")
  }
  
  # Combine base videos (from yesterday) with new API results
  all_results <- bind_rows(base_videos, api_results)
  ...
  if (nrow(all_results) > 0) {
    final_results <- all_results %>%
      ...
      filter(!is.na(video_description) & video_description != "") %>%
      distinct(video_id, .keep_all = TRUE)  # Remove duplicates by video_id
    
    # Log summary with proper user column detection
    log_info(sprintf("=== SMART ALL VIDEOS COLLECTION COMPLETED ==="))
    log_info(sprintf("Base videos (reused): %d", nrow(base_videos)))
    log_info(sprintf("Fresh API videos: %d", nrow(api_results)))
    log_info(sprintf("Total unique videos: %d", nrow(final_results)))
    
    # Use the correct user column name
    user_column <- if("author_name" %in% colnames(final_results)) "author_name" else "username"
    log_info(sprintf("Unique users: %d", n_distinct(final_results[[user_column]])))
