        ...
        tryCatch({
            ...
            if (httr::status_code(response) == 200) {
                ...
                if (length(content$data$videos) == 0) {
                    log_warn(paste("No videos found for chunk", chunk_index, "on attempt", attempts))
                ...
                log_info(paste("Retrieved", nrow(result), "videos for chunk", chunk_index, "on attempt", attempts))
                ...
                if (!is.null(content$data$has_more) && content$data$has_more) {
                    ...
                    log_info(paste("Pagination: Moving to page", pagination_attempts + 1, "for chunk", chunk_index))
                } else {
                ...
                log_warn(paste(error_message, "for chunk", chunk_index, "on attempt", attempts))
                global_error_list <<- c(global_error_list, list(list(
                    type = "api_error",
            ...
            log_warn(paste("An unexpected error occurred for chunk", chunk_index, "on attempt", attempts, ":", e$message))
            global_error_list <<- c(global_error_list, list(list(
...
tryCatch({
    ...
    recent_videos_list <- map2(chunks, chunk_indices, function(chunk, index) {
        tryCatch({
            if (length(chunk) == 0) {
                log_warn(paste("Empty chunk encountered at index", index))
                global_error_list <<- c(global_error_list, list(list(
                    type = "empty_chunk",
                    chunk_index = index
                )))
            ...
            log_warn(paste("Chunk content:", paste(chunk, collapse = ", ")))
            global_error_list <<- c(global_error_list, list(list(
