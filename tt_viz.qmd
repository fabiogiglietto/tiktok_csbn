...
generate_cluster_labels <- function(coordination_results, all_videos) {
  ...
  get_cluster_content <- function(cluster_id, max_videos = 20) {
    ...
    log_info(sprintf("Cluster %d has %d accounts", cluster_id, length(cluster_accounts)))
    ...
    log_info(sprintf("Found %d videos for cluster %d", nrow(cluster_videos), cluster_id))
    
    if (nrow(cluster_videos) == 0) {
      log_warn(sprintf("No videos found for cluster %d", cluster_id))
    ...
    log_info(sprintf("Generated content for cluster %d: %d characters", cluster_id, nchar(content)))
    return(content)
  }
  
  # Function to get label from OpenAI
  get_cluster_label <- function(content, cluster_id, retries = 3) {
    system_prompt <- "You are a researcher analyzing coordinated behavior on TikTok. 
Your task is to generate a concise (maximum 8 words), descriptive label in English that 
...
IMPORTANT: You must ALWAYS include the prevalent language as a two-letter country code in parentheses at the end of your label.
For example: 'Political Commentary (EN)', 'Entertainment Content (ES)', 'News Discussion (FR)', etc.
Use standard ISO 3166-1 alpha-2 country codes (EN for English, ES for Spanish, FR for French, DE for German, IT for Italian, PT for Portuguese, AR for Arabic, etc.)."
    
    user_prompt <- sprintf("Analyze the following content from a cluster of TikTok accounts and 
generate a concise label that describes their shared characteristics. Remember to include the prevalent language as a two-letter code in parentheses:
...
Label:", content)
    ...
    # Fix the model name
    model_name <- if (params$ai_model == "chatgpt-4o-latest") "gpt-4o" else params$ai_model
    log_info(sprintf("Using corrected model name: %s for cluster %d", model_name, cluster_id))
    
    for (attempt in 1:retries) {
      tryCatch({
        log_info(sprintf("API attempt %d for cluster %d", attempt, cluster_id))
        ...
        response <- httr::POST(
          ...
          body = list(
            model = model_name,  # Use corrected model name
            messages = list(
        ...
        log_info(sprintf("API response status for cluster %d: %d", cluster_id, status_code))
        
        if (status_code == 200) {
          content_response <- httr::content(response)
          label <- content_response$choices[[1]]$message$content
          
          # Ensure the label has language code - add fallback if missing
          if (!grepl("\\([A-Z]{2}\\)\\s*$", label)) {
            log_warn(sprintf("Label missing language code, adding (EN): %s", label))
            label <- paste0(label, " (EN)")
          }
          
          log_info(sprintf("Successfully generated label for cluster %d: '%s'", cluster_id, label))
          return(label)
        } else {
          error_content <- httr::content(response)
          log_error(sprintf("API request for cluster %d failed (status: %d): %s", 
                           cluster_id, status_code, 
                           if(is.list(error_content) && !is.null(error_content$error)) error_content$error$message else "Unknown error"))
        }
      }, error = function(e) {
        log_error(sprintf("Attempt %d for cluster %d failed: %s", attempt, cluster_id, conditionMessage(e)))
      })
      
      if (attempt < retries) {
        wait_time <- 2^attempt
        log_info(sprintf("Waiting %d seconds before retry for cluster %d", wait_time, cluster_id))
        Sys.sleep(wait_time)
      }
    }
    
    log_error(sprintf("All attempts failed for cluster %d, returning fallback label", cluster_id))
    return("Failed to generate label (EN)")
  }
  
  # Get number of components
  num_components <- components(coordination_results$coord_graph)$no
  log_info(sprintf("Number of components detected: %d", num_components))
  
  if (num_components == 0) {
    log_warn("No components found in coordination graph")
    return(data.frame(component = integer(), label = character(), stringsAsFactors = FALSE))
  }
  ...
  log_info(sprintf("Starting to process %d components", num_components))
  
  for (i in 1:num_components) {
    log_info(sprintf("Processing component %d of %d", i, num_components))
    
    # Get content for this cluster
    content <- get_cluster_content(i)
    
    # Get label
    label <- get_cluster_label(content, i)
    ...
    log_info(sprintf("Labels dataframe now has %d rows after processing component %d", nrow(labels), i))
  ...
  log_info(sprintf("Completed processing all components. Final labels dataframe has %d rows", nrow(labels)))
  
  # Save labels to file
  write_csv(labels, file.path(daily_output_dir, "cluster_labels.csv"))
  log_info("Saved cluster labels to file")
...
log_info("=== DEFINING NETWORK VISUALIZATION FUNCTION ===")
...
generate_network_visualization <- function(coordination_results,
  ...
  log_info("Starting network visualization function")
  ...
  tryCatch({
    ...
    log_info(sprintf("Created visualization directory: %s", viz_dir))
    
    # Get the graph
    graph <- coordination_results$coord_graph
    if (!igraph::is_igraph(graph)) {
      log_error("Invalid graph object provided")
      stop("Invalid graph object provided")
    }
    
    log_info(sprintf("Processing graph with %d nodes and %d edges", 
                     igraph::gorder(graph), igraph::gsize(graph)))
    
    # Convert to visNetwork format
    data <- visNetwork::toVisNetworkData(graph)
    log_info("Converted graph to visNetwork format")
    
    # Calculate component membership first
    ...
    # Enhanced tooltip with cluster/network ID
    data$nodes$title <- sprintf(
      "<div class='network-tooltip'><strong>@%s</strong><br>Network: %d<br>Connections: %d</div>",
      data$nodes$id,
    ...
    # Add cluster labels if available AND has data
    if(exists("cluster_labels") && 
       is.data.frame(get("cluster_labels")) && 
       nrow(get("cluster_labels")) > 0) {
      
      log_info("Adding cluster labels to visualization")
      ...
      tryCatch({
        label_lookup <- get("cluster_labels") %>%
          ...
          dplyr::select(component_id, group_label, label)
        
        # Only proceed if label_lookup has data
        if (nrow(label_lookup) > 0) {
          ...
          # Enhanced tooltip with cluster label when available
          data$nodes$title <- sprintf(
            "<div class='network-tooltip'><strong>@%s</strong><br>Network: %d<br>%s<br>Connections: %d</div>",
            data$nodes$id,
            data$nodes$component,
            ifelse(is.na(data$nodes$label), "", paste0("Label: ", data$nodes$label)),
            data$nodes$value
          )
          
          log_info("Successfully added cluster labels to nodes")
        } else {
          log_warn("cluster_labels exists but is empty, using default grouping")
          # Fall back to default grouping
          data$nodes$group <- sprintf("Network %d", comp)
          data$nodes$size <- data$nodes$value * node_size_multiplier + node_size_base
        }
      }, error = function(e) {
        log_warn(sprintf("Error adding cluster labels: %s. Using default grouping.", e$message))
        # Fall back to default grouping
        data$nodes$group <- sprintf("Network %d", comp)
        data$nodes$size <- data$nodes$value * node_size_multiplier + node_size_base
      })
      ...
      log_info("No cluster labels available or empty, using default node grouping")
      data$nodes$group <- sprintf("Network %d", comp)
      data$nodes$size <- data$nodes$value * node_size_multiplier + node_size_base
    }
    ...
    log_info("Creating visNetwork visualization")
    ...
    log_info(sprintf("Network visualization complete. Saved to: %s", html_file))
    log_info(sprintf("Network has %d nodes and %d edges", 
                     igraph::gorder(graph), igraph::gsize(graph)))
...
log_info("Network visualization function defined successfully")
```

``````{r generate_visualization, include=FALSE, message=FALSE}
log_info("=== GENERATING NETWORK VISUALIZATION ===")

# Debug coordination results
if (exists("coordination_results")) {
  log_info("✅ coordination_results exists")
  if (!is.null(coordination_results)) {
    log_info("✅ coordination_results is not NULL")
    if (!is.null(coordination_results$coord_graph)) {
      log_info("✅ coord_graph exists")
      if (igraph::is_igraph(coordination_results$coord_graph)) {
        log_info(sprintf("✅ Valid igraph with %d nodes and %d edges", 
                        igraph::gorder(coordination_results$coord_graph),
                        igraph::gsize(coordination_results$coord_graph)))
      } else {
        log_error("❌ coord_graph is not a valid igraph")
      }
    } else {
      log_error("❌ coord_graph is NULL")
    }
  } else {
    log_error("❌ coordination_results is NULL")
  }
} else {
  log_error("❌ coordination_results does not exist")
}

# Generate the visualization
network_viz <- tryCatch({
  if (exists("coordination_results") && !is.null(coordination_results$coord_graph)) {
    log_info("Calling generate_network_visualization function...")
    
    viz <- generate_network_visualization(
      coordination_results,                                  
    ...
    if (!is.null(viz)) {
      log_info("✅ Network visualization generated successfully")
    } else {
      log_warn("❌ Network visualization returned NULL")
    }
  ...
  log_error(sprintf("❌ Failed to generate network visualization: %s", e$message))
  log_error(sprintf("Error class: %s", class(e)[1]))
  log_error(sprintf("Error call: %s", deparse(e$call)))
  NULL
})
...
# Final status check
if (exists("network_viz")) {
  if (!is.null(network_viz)) {
    log_info("✅ network_viz object created and stored successfully")
  } else {
    log_warn("❌ network_viz is NULL after generation attempt")
  }
} else {
  log_error("❌ network_viz object was not created")
}

log_info("=== NETWORK VISUALIZATION GENERATION COMPLETED ===")
```
