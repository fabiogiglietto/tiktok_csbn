...
create_accounts_table <- function(summary_accounts, coord_graph) {
  ...
  datatable(accounts_table, 
    options = list(
      pageLength = 25, 
      scrollX = TRUE,
      order = list(list(5, 'desc')),  # Sort by Strength by default
      initComplete = JS(
...
create_clusters_table <- function(coord_graph, summary_accounts) {
  # Get components
  ...
  # Add account lists and average metrics
  clusters <- clusters %>%
    mutate(
  ...
  datatable(clusters,
    options = list(
      pageLength = 10, 
      scrollX = TRUE,
      order = list(list(1, 'asc')),  # Sort by Component by default
      initComplete = JS(
        "function(settings, json) {",
        "  $(this.api().table().container()).css({'font-size': '9px'});",
        "}")
...
create_posts_table <- function(summary_groups, all_videos) {
  ...
  datatable(posts_table,
    options = list(
      pageLength = 25, 
      scrollX = TRUE,
      order = list(list(3, 'desc')),  # Sort by Timestamp by default
      initComplete = JS(
        "function(settings, json) {",
        "  $(this.api().table().container()).css({'font-size': '9px'});",
        "}")
...
# Create and display all tables if data is available
