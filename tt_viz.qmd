...
create_clusters_table <- function(coord_graph, summary_accounts, all_videos) {
  ...
  clusters <- clusters %>%
    mutate(
      ...
      avg_time_delta = sapply(1:comp$no, function(i) {
        mean(summary_accounts$avg_time_delta[summary_accounts$account_id %in% 
             V(coord_graph)$name[comp$membership == i]], na.rm = TRUE)
      }),
      avg_edge_symmetry = sapply(1:comp$no, function(i) {
        mean(summary_accounts$avg_edge_symmetry_score[summary_accounts$account_id %in% 
             V(coord_graph)$name[comp$membership == i]], na.rm = TRUE)
      })
    ) %>%
    ...
    select(
      ...
      `Avg Time Between Shares (sec)` = avg_time_delta,
      `Avg Edge Symmetry` = avg_edge_symmetry,
      `Avg Views` = avg_views,
      `Avg Likes` = avg_likes,
      `Avg Comments` = avg_comments,
      `Avg Shares` = avg_shares,
