...
get_videos_by_description <- function(desc, start_date, end_date) {
  # Sanitize description for logging by escaping curly braces
  # This prevents glue parser errors while keeping the description content in logs
  log_safe_desc <- gsub("\\{", "{{", gsub("\\}", "}}", desc))
  
  # Truncate for display if too long
  log_display_desc <- if(nchar(log_safe_desc) > 100) {
    paste0(substr(log_safe_desc, 1, 100), "...")
  } else {
    log_safe_desc
  }
  
  log_info(sprintf("Querying for description: %s", log_display_desc))
  ...
  tryCatch({
    ...
    log_info(sprintf("Retrieved first page with %d results for description: %s", 
                    nrow(chunk_result), log_display_desc))
    ...
    log_error(sprintf("Error querying description '%s': %s", 
                     log_display_desc, conditionMessage(e)))
    global_error_list <<- c(global_error_list, list(list(
...
fetch_all_videos <- function(recent_videos, start_date, end_date) {
 ...
 # Safely log the count without any description content that might have braces
 ...
 for (desc in unique_descriptions) {
   ...
   if (is.data.frame(videos) && nrow(videos) > 0) {
     ...
     log_safe_desc <- gsub("\\{", "{{", gsub("\\}", "}}", desc))
     log_display_desc <- if(nchar(log_safe_desc) > 50) {
       paste0(substr(log_safe_desc, 1, 50), "...")
     } else {
       log_safe_desc
     }
     
     log_info(sprintf("Added %d videos for description: %s", nrow(videos), log_display_desc))
   }
