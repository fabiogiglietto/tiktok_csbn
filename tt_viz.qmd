...
Today, we have analyzed `r nrow(recent_videos)` videos from a set of `r length(coord_users)` monitored accounts. Those accounts posted `r nrow(all_videos)` between `r startdate` and `r enddate`. The coordinated detection analysis on these video identified `r nrow(summary_accounts)` accounts in `r igraph::components(coord_graph)$no` components and discovered `r nrow(new_account_ids)` new coordinated accounts.
