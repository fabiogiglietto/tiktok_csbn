...
# Set working directory using file.path instead of setwd
...
# Main setup function - FIXED to avoid setwd
setup <- function() {
...
# Function to get videos using traktok package with enhanced error handling
...
# FIXED get_videos_by_description function with correct field_values format
get_videos_by_description <- function(desc, start_date, end_date) {
  ...
  # First page with safe API call and CORRECTED field_values
  first_page_result <- safe_api_call(function() {
    query() |>
      query_and(
        field_name = "keyword",
        operation = "EQ",
        field_values = desc  # ✅ FIXED: Use desc directly, not list(desc) or c(desc)
      ) |>
      tt_search_api(
        start_date = start_date,
        end_date = end_date,
    ...
    full_results <- safe_api_call(function() {
      query() |>
        query_and(
          field_name = "keyword",
          operation = "EQ",
          field_values = desc  # ✅ FIXED: Use desc directly
        ) |>
        tt_search_api(
          start_date = start_date,
          end_date = end_date,
...
# Function to batch process descriptions and get videos with FIXED field_values
batch_fetch_videos <- function(descriptions, start_date, end_date, batch_size = 3, max_pages = 3) {
  ...
  log_info("Starting batched description queries with batch size %d", batch_size)
  
  # Split descriptions into batches
  batches <- split(descriptions, ceiling(seq_along(descriptions)/batch_size))
  
  log_info("Created %d batches from %d unique descriptions", length(batches), length(descriptions))
...
fetch_all_videos <- function(recent_videos, start_date, end_date) {
  ...
  if (is.null(recent_videos) || nrow(recent_videos) == 0) {
    log_warn("No recent videos available for description queries")
    return(data.frame())
  }
  ...
  # Log the total count without including any descriptions
  log_info(sprintf("Found %d unique descriptions to query", length(unique_descriptions)))
  
  # Process descriptions in batches with conservative settings
  all_results <- batch_fetch_videos(
    unique_descriptions, 
    start_date, 
    end_date,
    batch_size = 3,  # Very conservative
    max_pages = 3    # Very conservative
  )
  ...
  if (nrow(all_results) > 0) {
    ...
    # Log summary
    log_info(sprintf("Collection completed:"))
...
# FIXED: Enhanced coordination detection function with better data validation
# FIXED: Enhanced coordination detection function with better data validation
detect_coordination <- function(all_videos) {
  tryCatch({
    # Check if we have sufficient data
    if (is.null(all_videos) || nrow(all_videos) == 0) {
      log_warn("No videos available for coordination detection")
      return(create_empty_coordination_result())
    }
    
    if (!"video_description" %in% colnames(all_videos)) {
      log_warn("Missing video_description column")
      return(create_empty_coordination_result())
    }
    
    # ENHANCED: More thorough data cleaning and validation
    log_info("Starting data validation and cleaning")
    
    # Clean and filter the data
    clean_videos <- all_videos %>%
      # Remove rows with missing essential data
      filter(
        !is.na(video_description),
        !is.na(author_name),
        !is.na(video_id),
        !is.na(create_time),
        nchar(trimws(video_description)) > 0,
        nchar(trimws(author_name)) > 0,
        nchar(trimws(video_id)) > 0
      ) %>%
      # Clean text fields
      mutate(
        video_description = trimws(video_description),
    ...
    log_info(sprintf("Data cleaning: %d -> %d videos", nrow(all_videos), nrow(clean_videos)))
    
    # Check for minimum data requirements after cleaning
    unique_descriptions <- length(unique(clean_videos$video_description))
    unique_authors <- length(unique(clean_videos$author_name))
    
    log_info(sprintf("Data check: %d videos, %d unique descriptions, %d unique authors", 
                    nrow(clean_videos), unique_descriptions, unique_authors))
    
    if (unique_descriptions < params$min_participation || unique_authors < params$min_participation) {
      log_warn("Insufficient data for coordination detection after cleaning")
      return(create_empty_coordination_result())
    }
    
    # ENHANCED: Additional validation for duplicate content
    content_frequency <- clean_videos %>%
      group_by(video_description) %>%
      summarise(
        account_count = n_distinct(author_name),
        total_posts = n(),
        .groups = 'drop'
      ) %>%
      filter(account_count >= params$min_participation)
    
    if (nrow(content_frequency) == 0) {
      log_warn("No content shared by minimum required accounts")
      return(create_empty_coordination_result())
    }
    
    log_info(sprintf("Found %d pieces of content shared by %d+ accounts", 
                    nrow(content_frequency), params$min_participation))
    
    # Filter to only include videos with content that meets minimum participation
    analysis_videos <- clean_videos %>%
      filter(video_description %in% content_frequency$video_description)
    
    log_info(sprintf("Analysis dataset: %d videos from %d accounts", 
                    nrow(analysis_videos), n_distinct(analysis_videos$author_name)))
    
    # FIXED: Prepare data with enhanced error handling
    tryCatch({
      ...
      # Validate prep_data structure
      if (is.null(prep_data) || nrow(prep_data) == 0) {
        log_warn("prep_data is empty or NULL")
        return(create_empty_coordination_result())
      }
      
      log_info(sprintf("Prepared data: %d rows", nrow(prep_data)))
      
    }, error = function(e) {
      log_error(sprintf("Error in prep_data: %s", conditionMessage(e)))
      return(create_empty_coordination_result())
    })
    
    # FIXED: Detect coordinated groups with enhanced error handling
    tryCatch({
      ...
      log_info("Coordination detection completed")
      
      # Validate result
      if (is.null(result) || nrow(result) == 0) {
        log_warn("No coordinated groups detected by detect_groups")
        return(create_empty_coordination_result())
      }
      
      log_info(sprintf("Detected coordination: %d relationships", nrow(result)))
      
    }, error = function(e) {
      log_error(sprintf("Error in detect_groups: %s", conditionMessage(e)))
      return(create_empty_coordination_result())
    })
    
    # FIXED: Generate network with enhanced error handling
    tryCatch({
      ...
      if (is.null(coord_graph) || !igraph::is_igraph(coord_graph)) {
        log_warn("Failed to generate coordination network")
        return(create_empty_coordination_result())
      }
      
      # Check if graph has nodes and edges
      if (igraph::vcount(coord_graph) == 0) {
        log_warn("Generated network has no nodes")
        return(create_empty_coordination_result())
      }
      
      log_info(sprintf("Network generated: %d nodes, %d edges", 
                      igraph::vcount(coord_graph), igraph::ecount(coord_graph)))
      
    }, error = function(e) {
      log_error(sprintf("Error in generate_coordinated_network: %s", conditionMessage(e)))
      return(create_empty_coordination_result())
    })
    
    # FIXED: Calculate summary statistics with enhanced error handling
    tryCatch({
      ...
      if (is.null(summary_groups) || is.null(summary_accounts)) {
        log_warn("Failed to generate summary statistics")
        return(create_empty_coordination_result())
      }
      
      if (nrow(summary_accounts) == 0) {
        log_warn("No accounts in summary statistics")
        return(create_empty_coordination_result())
      }
      ...
      log_error(sprintf("Error in summary statistics: %s", conditionMessage(e)))
      return(create_empty_coordination_result())
    })
    ...
    tryCatch({
      ...
      log_info("Successfully saved coordination results")
    }, error = function(e) {
      log_warn(sprintf("Error saving coordination results: %s", conditionMessage(e)))
    })
    ...
    log_error(paste("Error in coordinated detection:", conditionMessage(e)))
    global_error_list <<- c(global_error_list, list(list(
...
# ENHANCED: Helper function to create empty coordination result
create_empty_coordination_result <- function() {
  return(list(
    coord_graph = NULL,
    summary_groups = data.frame(
      object_id = character(),
      num_accounts = numeric(),
      stringsAsFactors = FALSE
    ),
    summary_accounts = data.frame(
      account_id = character(),
      avg_time_delta = numeric(),
      avg_edge_symmetry_score = numeric(),
      unique_shares_count = numeric(),
      stringsAsFactors = FALSE
    ), 
    ...
    updated_list = if(exists("coord_users")) coord_users else character()
  ))
}
...
if (exists("all_videos") && is.data.frame(all_videos) && nrow(all_videos) > 0) {
  ...
  coordination_results <- create_empty_coordination_result()
}
...
# Plot historical trends only if we have data
if (nrow(historical_data$logs) > 0) {
  ...
  cat("No historical data available for trend visualization.")
}
```
...
# Function to create empty placeholder table
create_empty_table <- function(message = "No data available") {
  empty_data <- data.frame(
    Status = message,
    stringsAsFactors = FALSE
  )
  return(create_datatable_base(empty_data))
}
...
create_posts_table <- function(summary_groups, all_videos, coord_graph) {
  tryCatch({
    ...
    log_warn(sprintf("Error creating posts table: %s", e$message))
    return(create_empty_table("No coordinated posts data available"))
  })
}
...
create_accounts_table <- function(summary_accounts, coord_graph, all_videos) {
  tryCatch({
    ...
    log_warn(sprintf("Error creating accounts table: %s", e$message))
    return(create_empty_table("No coordinated accounts data available"))
  })
}
...
create_clusters_table <- function(coord_graph, summary_accounts, all_videos) {
  tryCatch({
    ...
    log_warn(sprintf("Error creating clusters table: %s", e$message))
    return(create_empty_table("No coordinated clusters data available"))
  })
}

# FIXED: Create all tables with proper error handling
tryCatch({
  if (exists("coordination_results") && 
      !is.null(coordination_results) && 
      !is.null(coordination_results$coord_graph) &&
      !is.null(coordination_results$summary_accounts) &&
      !is.null(coordination_results$summary_groups) &&
      exists("all_videos") && 
      is.data.frame(all_videos) && 
      nrow(all_videos) > 0) {
    
    log_info("Creating coordination analysis tables")
    ...
    log_info("Successfully created all coordination tables")
    
  } else {
    # Create empty tables when no coordination data is available
    log_warn("No coordination results available - creating empty tables")
    
    accounts_table <- create_empty_table("No coordination analysis was performed due to insufficient data")
    clusters_table <- create_empty_table("No coordination networks were detected") 
    posts_table <- create_empty_table("No coordinated posts were identified")
  }
}, error = function(e) {
  log_error(sprintf("Error in table creation: %s", e$message))
  
  # Fallback: create empty tables
  accounts_table <- create_empty_table("Error occurred during table creation")
  clusters_table <- create_empty_table("Error occurred during table creation")
  posts_table <- create_empty_table("Error occurred during table creation")
})

# Ensure tables exist in global environment
if (!exists("accounts_table")) {
  accounts_table <- create_empty_table("Table creation failed")
}
if (!exists("clusters_table")) {
  clusters_table <- create_empty_table("Table creation failed")  
}
if (!exists("posts_table")) {
  posts_table <- create_empty_table("Table creation failed")
}
```

```{r display_tables_fixed, echo=FALSE, results='asis'}
# Enhanced table display with error handling
cat("## Coordination Analysis Results\n\n")

# Check if we have meaningful coordination results
has_coordination_data <- exists("coordination_results") && 
                        !is.null(coordination_results) &&
                        !is.null(coordination_results$coord_graph) &&
                        exists("all_videos") &&
                        is.data.frame(all_videos) &&
...
if (has_coordination_data) {
  cat("✅ **Coordination analysis completed successfully**\n\n")
  cat(sprintf("- **Networks detected:** %d\n", components(coordination_results$coord_graph)$no))
  cat(sprintf("- **Coordinated accounts:** %d\n", nrow(coordination_results$summary_accounts)))
  cat(sprintf("- **New accounts found:** %d\n\n", nrow(coordination_results$new_accounts)))
} else {
  cat("⚠️ **No coordination patterns detected**\n\n")
  cat("The analysis was unable to identify coordinated behavior patterns due to:\n")
  cat("- Insufficient video data collected\n")
  cat("- API quota limitations\n") 
  cat("- Data quality issues\n\n")
  cat("*Empty tables are shown below for reference.*\n\n")
}
```

### Coordinated Accounts
```{r display_accounts_fixed, echo=FALSE}
if (exists("accounts_table")) {
  accounts_table
} else {
  cat("Accounts table not available")
}
```

### Coordinated Networks/Clusters  
```{r display_clusters_fixed, echo=FALSE}
if (exists("clusters_table")) {
  clusters_table
} else {
  cat("Clusters table not available")
}
```

### Coordinated Posts
```{r display_posts_fixed, echo=FALSE}  
if (exists("posts_table")) {
  posts_table
} else {
  cat("Posts table not available")
}
```
...
prepare_download_files <- function() {
  ...
  # Save tables as CSV only if they exist and have data
  tryCatch({
    if (exists("accounts_table") && !is.null(accounts_table$x$data)) {
      write.csv(accounts_table$x$data, "coordinated_accounts.csv", row.names = FALSE)
    }
    if (exists("clusters_table") && !is.null(clusters_table$x$data)) {
      write.csv(clusters_table$x$data, "network_clusters.csv", row.names = FALSE)
    }
    if (exists("posts_table") && !is.null(posts_table$x$data)) {
      write.csv(posts_table$x$data, "coordinated_posts.csv", row.names = FALSE)
    }
  }, error = function(e) {
    log_warn(sprintf("Error saving CSV files: %s", e$message))
  })
  ...
  tryCatch({
    ...
    log_warn(sprintf("Error saving network graph: %s", e$message))
  })
...
Giglietto, F., Marino, G., Mincigrucci, R., & Stanziano, A. (2023). A Workflow to Detect, Monitor, and Update Lists of Coordinated Social Media Accounts Across Time: The Case of the 2022 Italian Election. Social Media + Society, 9(3).'
  ...
  # Explicitly list only the files we want to include
  files_to_zip <- c(
    "README.md",
    "coordinated_accounts.csv",
    "network_clusters.csv",
    "coordinated_posts.csv",
    "coordination_network.graphml"
  )
...
# Create the zip file only if we have coordination results
if (has_coordination_data) {
  ...
  cat("No coordination results available for download.")
}
