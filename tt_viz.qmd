...
# Function to perform the API query with retry logic and error counting
get_recent_videos <- function(max_retries = 3) {
  attempts <- 0
  repeat {
    attempts <- attempts + 1
    tryCatch({
      data <- query() |>
        query_and(field_name = "username", operation = "IN", field_values = coord_users) |>
        tt_search_api(start_date = startdate, end_date = enddate, max_pages = 200L, verbose = TRUE, fields = "all")
      ...
      if (grepl("rate limit", e$message, ignore.case = TRUE) && attempts <= max_retries) {
        Sys.sleep(60)  # Sleep for a minute before retrying
      } else {
        return(list(error = TRUE, message = ifelse(grepl("rate limit", e$message, ignore.case = TRUE),
                                                   "The API rate limit was exceeded. Please try again later.",
                                                   paste("An unexpected error occurred:", e$message))))
      }
    })
    # Control to continue the loop or exit
...
if (is.list(recent_videos) && isTRUE(recent_videos$error)) {
  # Display the error message using knitr::kable for better formatting in outputs
  knitr::kable(data.frame(Date = Sys.Date(), Message = recent_videos$message), caption = "Error Details")
} else if (is.data.frame(recent_videos) && all(recent_videos$error == FALSE)) {
  # Process the data only if there was no error and the data frame is valid
...
for (i in 1:nrow(unique_videos_desc)) {
  result <- tryCatch({
    ...
    temp_videos <- query() |>
      query_and(field_name = "keyword", operation = "EQ", field_values = desc) |>
      tt_search_api(start_date = startdate, end_date = enddate, max_pages = 100L, verbose = TRUE, fields = "all")
    
    # Optional: Pause between API calls to avoid rate limits
    Sys.sleep(time = 2)  # Adjust time as needed based on the API's rate limit policy
    
    temp_videos  # Let this be the result returned on success

  }, error = function(e) {
    # Log the error message
    message(sprintf("Error fetching videos for description '%s': %s", desc, e$message))
    NULL  # Use NULL to indicate an error occurred
  })

  # Append either fetched data or NULL (in case of an error) to the list
  all_videos_list[[i]] <- result
}

# Normalize data frames in the list
all_videos_list <- lapply(all_videos_list, function(df) {
  if (!is.null(df) && "video_id" %in% names(df)) {
    df$video_id <- as.character(df$video_id)
  } else {
    df <- data.frame(video_id = character())  # Create an empty data frame with the 'video_id' column if df is NULL or 'video_id' is not present
  }
  return(df)
})
...
all_videos <- all_videos %>%
  ...
  dplyr::filter(video_description != "" & nchar(video_description) >= 80) %>%
  dplyr::distinct() %>%
...
if (is.list(recent_videos) && any(recent_videos$error)) {
  cat("Due to an API rate limit issue, the most recent video data could not be retrieved. Please see the error log for more details.")
} else {
  # Construct the message assuming data retrieval was successful
  message <- sprintf("We retrieved %s videos from %s monitored accounts, during the period from %s to %s. Using a unique list of video descriptions from these accounts, we accessed a total of %s videos posted on TikTok within the same timeframe. Our analysis for coordinated detection in these videos identified %s accounts spread across %s components, and it also uncovered %s new accounts exhibiting coordinated behavior.",
