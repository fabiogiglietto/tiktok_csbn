  ...
  tryCatch({
    ...
    data$nodes <- data$nodes %>%
      mutate(
        ...
        # First store the component number
      ...
      mutate(
        # Create the group label using component_id
        group = sprintf("%d - %s", component_id, ifelse(is.na(label), "No Label", label)),
    ...
    network <- visNetwork(
      ...
      # Then update the visOptions part:
    visOptions(
      ...
      selectedBy = list(
        variable = "group",
        main = "Select Component - Label"  # Updated title
      )
    ) %>%
      visPhysics(
        stabilization = list(
...
create_clusters_table <- function(coord_graph, summary_accounts, all_videos) {
  ...
  clusters <- clusters %>%
    ...
    mutate(
      # Format Component and Label together
      Component = sprintf("%d. %s", Component, ifelse(is.na(Label), "No Label", Label)),
