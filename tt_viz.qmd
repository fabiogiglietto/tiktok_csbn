...
recent_videos <- tryCatch({
  ...
  # Ensure 'error' is always initialized
  data$error <- FALSE
  data
}, error = function(e) {
  # Returning a simple list with an error message to check later
  list(error = TRUE, message = "The API rate limit was exceeded. Please try again later.")
})

# Check if recent_videos contains error information
if (is.list(recent_videos) && "error" %in% names(recent_videos) && any(recent_videos$error)) {
  knitr::kable(data.frame(Date = Sys.Date(), Message = recent_videos$message))
} else if (is.data.frame(recent_videos) && !any(recent_videos$error)) {
  # Process data frame only if no error attribute set to TRUE
  videos_with_same_desc <- recent_videos %>%
    dplyr::mutate(video_description = tolower(video_description)) %>%
    dplyr::group_by(video_description) %>%
    dplyr::filter(dplyr::n() >= 3) %>%
    dplyr::ungroup()
  
  unique_videos_desc <- videos_with_same_desc %>%
    dplyr::select(video_description) %>%
