...
# Smart Video Reuse Mechanism for TikTok Data Collection

```{r smart_reuse_setup, include=FALSE, message=FALSE}

# Function to calculate date overlap for video filtering
...
# Updated fetch_recent_videos function with smart reuse
fetch_recent_videos_smart <- function() {
  if (!params$load_from_snapshot) {
    log_info("=== STARTING SMART VIDEO COLLECTION ===")
    
    # Check if we have smart reuse results
...
# Enhanced fetch_all_videos function that leverages smart reuse
...
fetch_all_videos_smart <- function(recent_videos, overall_start_date, overall_end_date) { # Renamed params for clarity
  ...
  if (file.exists(yesterday_paths$snapshot)) {
    tryCatch({
      ...
      # Add ALL columns you expect in your snapshot, otherwise they might be dropped or mis-typed
      ...
      # Filter yesterday's data to the current OVERALL date range
      if (nrow(yesterday_full_data) > 0 && "create_time" %in% colnames(yesterday_full_data)) {
      ...
      log_info(sprintf("Loaded %d videos from yesterday's snapshot (within overall date range) as base_videos", nrow(base_videos)))
    }, error = function(e) {
      log_warn(sprintf("Could not load or process yesterday's full data: %s. Starting with empty base_videos.", e$message))
  ...
  # 2. Identify Target Descriptions from recent_videos
  if (is.null(recent_videos) || nrow(recent_videos) == 0) {
    log_warn("No recent_videos available to identify target descriptions.")
    if (nrow(base_videos) > 0) {
      log_info("Processing and returning only base_videos as recent_videos is empty/null.")
      # Ensure 'author_name' and 'video_id' exist before creating video_url
      final_results <- base_videos %>%
        mutate(
          region_code = if("region_code" %in% colnames(.)) toupper(region_code) else NA_character_,
          video_url = if("author_name" %in% colnames(.) && "video_id" %in% colnames(.)) paste0("https://www.tiktok.com/@", author_name, "/video/", video_id) else NA_character_
        ) %>%
  ...
  if (length(target_descriptions) == 0) {
    ...
    if (nrow(base_videos) > 0) {
      log_info("Processing and returning only base_videos as no target_descriptions identified.")
      final_results <- base_videos %>%
        mutate(
          region_code = if("region_code" %in% colnames(.)) toupper(region_code) else NA_character_,
          video_url = if("author_name" %in% colnames(.) && "video_id" %in% colnames(.)) paste0("https://www.tiktok.com/@", author_name, "/video/", video_id) else NA_character_
  ...
  log_info(sprintf("Identified %d unique target descriptions from recent_videos.", length(target_descriptions)))
  ...
  for (desc_val in target_descriptions) {
    ...
    if (nrow(cached_videos_for_this_desc) > 0) {
      ...
      if (is.finite(max_cached_create_time_posix)) {
        ...
        log_info(sprintf("Desc: '%.30s...': Max cached date %s. New query start for API: %s",
                       desc_val, format(as.Date(max_cached_create_time_posix), "%Y-%m-%d"), 
                       format(api_query_start_date_for_this_desc, "%Y-%m-%d")))
      } else {
         log_info(sprintf("Desc: '%.30s...': No valid max_cached_create_time. Defaulting query start to overall_start_date: %s",
                       desc_val, format(api_query_start_date_for_this_desc, "%Y-%m-%d")))
      }
    } else {
      log_info(sprintf("Desc: '%.30s...': Not found in cache. Querying from overall_start_date: %s",
                     desc_val, format(api_query_start_date_for_this_desc, "%Y-%m-%d")))
    }
    ...
    if (api_query_start_date_for_this_desc <= as.Date(overall_end_date)) {
      descriptions_to_query_api[[length(descriptions_to_query_api) + 1]] <- list(
       ...
       log_info(sprintf("Desc: '%.30s...': Calculated API query start date %s is after overall_end_date %s. Skipping API call.",
                       desc_val, format(api_query_start_date_for_this_desc, "%Y-%m-%d"), 
                       format(as.Date(overall_end_date), "%Y-%m-%d")))
    }
  ...
  if (length(descriptions_to_query_api) > 0) {
    log_info(sprintf("Need to query API for %d description/date-range pairs.", length(descriptions_to_query_api)))
    ...
    for (query_item in descriptions_to_query_api) {
      ...
      log_info(sprintf("API Call for Desc: '%.30s...', API Start: %s, API End: %s",
                     query_item$description, 
                     format(current_api_query_start_date, "%Y-%m-%d"), 
                     format(current_api_query_end_date, "%Y-%m-%d")))
      ...
      tryCatch({
          ...
          log_error(sprintf("Error fetching for desc '%.30s...': %s", query_item$description, e$message))
  ...
  if (length(newly_fetched_videos_list) > 0) {
    ...
    log_info(sprintf("Fetched a total of %d new videos via API for descriptions.", nrow(all_newly_fetched_videos_df)))
  }

  # 5. Combine Cached Videos with Newly Fetched Videos
  # Ensure base_videos is standardized if it's not empty and hasn't been yet.
  # standardize_data_types was already called on base_videos if it was loaded.
  ...
  if (nrow(combined_videos) > 0) {
    ...
    log_info(sprintf("=== INCREMENTAL 'ALL VIDEOS' COLLECTION COMPLETED ==="))
    log_info(sprintf("Base videos (reused from cache, within overall date range): %d", nrow(base_videos)))
    log_info(sprintf("Fresh API videos (incrementally fetched for descriptions): %d", nrow(all_newly_fetched_videos_df)))
    log_info(sprintf("Total unique videos after combination and deduplication: %d", nrow(final_results)))
    ...
    if (user_column_name != "video_id" || !"author_name" %in% colnames(final_results)) { # Check if a reasonable user column exists
        log_info(sprintf("Unique users (using '%s'): %d", user_column_name, n_distinct(final_results[[user_column_name]])))
    }
    if ("video_description" %in% colnames(final_results)) {
        log_info(sprintf("Unique descriptions in final set: %d", n_distinct(final_results$video_description)))
    }
    ...
    log_info(sprintf("Results saved to %s and %s", latest_snapshot_file, daily_snapshot_file_path))
