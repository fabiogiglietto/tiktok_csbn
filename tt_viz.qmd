...
get_network_label <- function(component, cluster_labels = NULL) {
  network_id <- sprintf("Network %d", component)
  if (!is.null(cluster_labels) && component %in% cluster_labels$component) {
    label <- cluster_labels$label[cluster_labels$component == component]
    if (!is.na(label) && label != "") {
      return(sprintf("%s: %s", network_id, label))
    }
  }
  return(network_id)
}
...
create_datatable_base <- function(data, pageLength = 10, fontSize = "9px", 
  ...
  if (!is.null(orderCol)) {
    options$order <- list(list(orderCol - 1, 'desc'))  # -1 because JS is 0-based
  }
...
get_network_metrics <- function(coord_graph) {
  list(
    degree = degree(coord_graph),
    strength = strength(coord_graph),
    betweenness = betweenness(coord_graph, normalized = TRUE),
    components = components(coord_graph)
  )
}

get_engagement_metrics <- function(data, group_by_col) {
  data %>%
...
create_accounts_table <- function(summary_accounts, coord_graph, all_videos) {
  # Get network metrics
  network_metrics <- get_network_metrics(coord_graph)
  ...
  engagement_metrics <- get_engagement_metrics(all_videos, "author_name")
  
  # Create final table
  accounts_table <- summary_accounts %>%
    mutate(
      account_link = map_chr(account_id, ~create_url_link(.x, "profile")),
      component = network_metrics$components$membership[match(account_id, V(coord_graph)$name)]
    ) %>%
  ...
  create_datatable_base(accounts_table, pageLength = 25, orderCol = 6)
}
...
create_clusters_table <- function(coord_graph, summary_accounts, all_videos) {
  ...
  engagement_metrics <- all_videos %>%
    mutate(
      component = comp$membership[match(author_name, V(coord_graph)$name)]
    ) %>%
    get_engagement_metrics("component")
  
  # Create final table
  clusters <- cluster_metrics %>%
    left_join(engagement_metrics, by = "component") %>%
    mutate(
      Network = map_chr(component, ~get_network_label(.x, get("cluster_labels", envir = .GlobalEnv))),
  ...
  create_datatable_base(clusters, fontSize = "12px") %>%
    formatStyle(
      columns = 2:9,
      className = 'dt-right'
    ) %>%
...
create_posts_table <- function(summary_groups, all_videos, coord_graph) {
  # Get component membership for each account first
  ...
  posts_table <- summary_groups %>%
    ...
    # Join with components data
    left_join(components_df, by = "author_name") %>%
    mutate(
      network_id = ifelse(!is.na(component), 
                         sapply(component, get_network_label, 
                               cluster_labels = get("cluster_labels", envir = .GlobalEnv)),
                         "Network NA"),
  ...
  create_datatable_base(posts_table, orderCol = 4)
}
