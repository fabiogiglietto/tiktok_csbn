    ...
    if(exists("cluster_labels") && is.data.frame(cluster_labels) && nrow(cluster_labels) > 0) {
      ...
      label_lookup <- cluster_labels %>%
        mutate(
          ...
          group_label = sprintf("%d - %s", component_id, label)
        ) %>%
      ...
      data$nodes <- data$nodes %>%
        ...
        mutate(
          group = coalesce(group_label, as.character(component_id))
        )
    } else {
      data$nodes <- data$nodes %>%
        mutate(
          group = as.character(component_id)
        )
    }
...
if (!is.null(network_results)) {
    log_info(sprintf(
      ...
      gorder(coordination_results$coord_graph),  # number of nodes
      gsize(coordination_results$coord_graph)    # number of edges
    ))
  } else {
