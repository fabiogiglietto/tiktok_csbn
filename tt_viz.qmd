...
# Main setup function - ONLY ONE DEFINITION
setup <- function() {
...
# Run setup directly - THIS LINE IS CRUCIAL
setup()
```
...
get_recent_videos <- function(usernames, start_date, end_date, chunk_size = 50, max_pages = 10) {
  ...
  # Process each chunk with a single request approach
  for (i in seq_along(username_chunks)) {
    ...
    tryCatch({
      # Make a single request with the specified max_pages
      chunk_result <- query() |>
        query_and(
          field_name = "username",
          operation = "IN",
...
fetch_recent_videos <- function() {
  if (!params$load_from_snapshot) {
    ...
    recent_videos_result <- get_recent_videos(
      ...
      end_date = enddate,
      max_pages = 10  # Fixed to 10 pages as requested
    )
...
batch_fetch_videos <- function(descriptions, start_date, end_date, batch_size = 5, max_pages = 10) {
  ...
  for (i in seq_along(batches)) {
    ...
    if (batch_count > 1) {
      tryCatch({
        # Build OR conditions for each description
        query_obj <- query()
        
        # Add OR conditions for each description
        or_conditions <- list()
        for (desc in current_batch) {
          or_conditions <- c(or_conditions, list(
            list(
              field_name = "keyword",
              operation = "EQ",
        ...
        # Execute batch query - first try with one page
        log_info(sprintf("Executing batch query for %d descriptions", batch_count))
        batch_first_page <- tt_search_api(
          query_obj,
          start_date = start_date,
          end_date = end_date,
          max_pages = 1,
        ...
        # If first page successful, try to get more pages
        if (is.data.frame(batch_first_page) && nrow(batch_first_page) > 0) {
          log_info(sprintf("Batch query first page returned %d videos", nrow(batch_first_page)))
          
          # Try to get more pages
          batch_results <- batch_first_page  # Default to first page results
          
          tryCatch({
            # Try to get more pages
            full_results <- tt_search_api(
              query_obj,
              start_date = start_date,
              end_date = end_date,
              max_pages = max_pages,
            ...
            # If successful, use full results
            if (is.data.frame(full_results) && nrow(full_results) > 0) {
              batch_results <- full_results
              log_info(sprintf("Successfully retrieved all pages with %d total videos", 
                              nrow(batch_results)))
            }
          }, error = function(e) {
            if (grepl("400|invalid_params|Search Id.*invalid or expired", e$message)) {
              ...
              global_error_list <<- c(global_error_list, list(list(
                type = "batch_pagination_error",
                batch = i,
                message = e$message
              )))
            } else {
              log_error(sprintf("Unexpected error during batch pagination: %s", conditionMessage(e)))
            }
          })
        ...
        # No need for explicit fallback - we'll process all descriptions that didn't return results below
      })
    }
    
    # For any descriptions that didn't return results in the batch query, process individually
    # Use a set to track which descriptions have been found in all_results
    ...
    for (desc in current_batch) {
      ...
      if (!desc %in% found_descriptions) {
        ...
        if (is.data.frame(videos) && nrow(videos) > 0) {
          ...
          # Use video_ids for logging instead of descriptions
...
# Keep the existing get_videos_by_description function for individual queries
get_videos_by_description <- function(desc, start_date, end_date) {
  ...
  tryCatch({
    ...
    if (is.data.frame(chunk_result) && nrow(chunk_result) > 0) {
      sample_video_id <- chunk_result$video_id[1]
      log_info(sprintf("Retrieved first page with %d results (sample video_id: %s)", 
                      nrow(chunk_result), sample_video_id))
    } else {
      log_info(sprintf("Retrieved first page with 0 results"))
    }
    ...
    tryCatch({
      full_results <- query() |>
        ...
        tt_search_api(
          start_date = start_date,
          end_date = end_date,
          max_pages = 10  # Reduced from 100 to be more conservative
        )
    ...
    log_error(sprintf("Error querying description: %s", conditionMessage(e)))
    global_error_list <<- c(global_error_list, list(list(
