...
```{r setup_tables, include=FALSE, message=FALSE}
...
create_accounts_table <- function(summary_accounts, coord_graph, all_videos) {
  ...
  DT::datatable(
    ...
    options = list(
      pageLength = 25,
      scrollX = TRUE,
      order = list(list(5, 'desc')), # Sort by degree by default
      initComplete = JS(
      ...
      columnDefs = list(
        list(className = 'dt-right', targets = 1:11), # Right-align numeric columns
        list(width = '200px', targets = 0), # Adjust Account column width
...
create_posts_table <- function(summary_groups, all_videos) {
  # Create posts table from summary_groups 
  # Note: summary_groups should already contain the primary metrics and grouping info
  DT::datatable(
    summary_groups,
    escape = FALSE,
    rownames = FALSE,
    options = list(
      pageLength = 10,
      scrollX = TRUE,
      order = list(list(4, 'desc')), # Sort by views by default
      initComplete = JS(
        "function(settings, json) {",
        "$(this.api().table().container()).css({'font-size': '12px'});",
        "}"
