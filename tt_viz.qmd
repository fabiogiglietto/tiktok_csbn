...
prepare_download_files <- function() {
  # Create download directory with full path
  download_dir <- file.path(getwd(), "daily_output", "download")
  ...
  # Copy network visualization
  viz_source <- file.path(getwd(), "daily_output", "network", "coordination_network.html")
  if (file.exists(viz_source)) {
    file.copy(viz_source,
  ...
  zip_file <- file.path(getwd(), "daily_output", 
                       sprintf("coordination_results_%s.zip", current_date))
  ...
  # Create zip from download directory contents
  files_to_zip <- list.files(download_dir, full.names = TRUE)
  if (length(files_to_zip) > 0) {
    utils::zip(zip_file, files = files_to_zip, flags = "-r9X")
  }
  
  # Clean up download directory
  unlink(download_dir, recursive = TRUE)
  
  return(if (file.exists(zip_file)) zip_file else NULL)
}
...
if (file.exists(zip_file)) {
  download_url <- file.path("daily_output", paste0("coordination_results_", format(Sys.Date(), "%Y-%m-%d"), ".zip"))
