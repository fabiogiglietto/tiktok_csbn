...
# Enhanced coordination detection function
detect_coordination <- function(all_videos) {
  tryCatch({
    ...
    unique_descriptions <- length(unique(all_videos$video_description))
    unique_authors <- length(unique(all_videos$author_name))
    
    log_info(sprintf("Data check: %d videos, %d unique descriptions, %d unique authors", 
                    nrow(all_videos), unique_descriptions, unique_authors))
    
    if (unique_descriptions < params$min_participation || unique_authors < params$min_participation) {
      log_warn("Insufficient data for coordination detection")
      return(create_empty_coordination_result())
    }
    ...
    if (is.null(result) || nrow(result) == 0) {
      log_warn("No coordinated groups detected")
      return(create_empty_coordination_result())
    }
...
# Helper function to create empty coordination result
