...
request_access_token <- function(client_key = NULL, client_secret = NULL) {
  # Endpoint URL
  url <- "https://open.tiktokapis.com/v2/oauth/token/"
  
  # Check if client_key and client_secret are NULL, and if so, use default values
  if (is.null(client_key)) {
    client_key <- Sys.getenv("TIKTOK_CLIENT_KEY")
    if (is.null(client_key)) {
      log_error("TIKTOK_CLIENT_KEY is not set")
      stop("TIKTOK_CLIENT_KEY is not set. Please provide a valid client key or set the environment variable.")
    }
  }
  ...
  if (is.null(client_secret)) {
    ...
    if (is.null(client_secret)) {
      log_error("TIKTOK_CLIENT_SECRET is not set")
      stop("TIKTOK_CLIENT_SECRET is not set. Please provide a valid client secret or set the environment variable.")
    }
  }
  ...
  tryCatch({
    ...
    if (response$status_code == 200) {
      ...
      log_error(paste("Failed to retrieve access token. Status code:", response$status_code))
      stop(paste("Failed to retrieve access token. Status code:", response$status_code))
    }
  }, error = function(e) {
...
# Set your client access token and other parameters
client_key <- Sys.getenv("TIKTOK_CLIENT_KEY")
client_secret <- Sys.getenv("TIKTOK_CLIENT_SECRET")
...
if (!params$load_from_snapshot) {
  ...
  tryCatch({
    access_token <- request_access_token(client_key, client_secret)
  }, error = function(e) {
...
get_recent_videos <- function(user_chunk, startdate, enddate, access_token, max_retries = 3, chunk_index) {
    attempts <- 0
    all_results <- list()
        ...
        query = list(
            and = list(
                list(
                    operation = "IN",
                    field_name = "username",
                    field_values = as.list(user_chunk)
                )
            )
        ),
    ...
    repeat {
        attempts <- attempts + 1
        log_info(paste("Starting attempt", attempts, "for chunk", chunk_index))
            ...
            if (httr::status_code(response) == 200) {
                ...
                if (length(content$data$videos) == 0) {
                    if (length(all_results) == 0) {
                        log_warn(paste("No data returned from API for chunk", chunk_index))
                        return(data.frame())
                    } else {
                        break  # Exit the loop if no more videos
                    }
                }
                ...
                log_info(paste("Retrieved", nrow(result), "videos for chunk", chunk_index))
                ...
                log_warn(paste("API request failed with status code:", httr::status_code(response), "for chunk", chunk_index))
                if (attempts >= max_retries) {
                    return(data.frame())
                }
            } else {
                log_error(paste("Unexpected API response:", httr::status_code(response), "for chunk", chunk_index))
        ...
        if (attempts >= max_retries) {
            log_warn(paste("Max retries exceeded for chunk", chunk_index, "after", max_retries, "attempts"))
            break
        }
    }
...
# Process to fetch and combine recent videos from chunks
chunks <- split(coord_users, ceiling(seq_along(coord_users) / 100))
chunk_indices <- seq_along(chunks)
failure_count <- 0
total_chunks <- length(chunks)

tryCatch({
    pb <- progress_bar$new(format = "[:bar] :percent :etas", total = length(chunks), clear = FALSE)
    
    recent_videos_list <- map2(chunks, chunk_indices, function(chunk, index) {
    tryCatch({
        videos <- get_recent_videos(chunk, startdate, enddate, access_token = access_token, max_retries = 10, chunk_index = index)
        if (nrow(videos) == 0) {
            failure_count <<- failure_count + 1
            log_warn(paste("Failed to retrieve videos for chunk", index))
            global_error_list <<- c(global_error_list, list(list(type = "chunk_retrieval", index = index, message = "No videos retrieved")))
        }
        return(videos)
    }, error = function(e) {
        failure_count <<- failure_count + 1
        log_warn(paste("Error in processing chunk", index, ":", e$message))
        global_error_list <<- c(global_error_list, list(list(type = "chunk_processing", index = index, message = e$message)))
        return(data.frame())
    })
})
...
recent_videos <- bind_rows(recent_videos_list)
    if (length(recent_videos_list) == 0) {
        log_warn("All API queries failed. Proceeding with empty recent_videos_list.")
    ...
    global_error_list <<- c(global_error_list, list(list(type = "recent_videos_overall", message = e$message)))
    recent_videos <- data.frame()
})

log_info(paste("Number of API failures:", failure_count, "out of", total_chunks, "chunks"))
api_failure_summary <- list(failures = failure_count, total_chunks = total_chunks)
...
if (nrow(recent_videos) > 0) {
    ...
    log_warn("No videos with matching criteria found")
    unique_videos_desc <- data.frame(video_description = character(0))
}

# Add a summary of errors to global_error_list
global_error_list <<- c(global_error_list, list(list(type = "api_failure_summary", failures = failure_count, total_chunks = total_chunks)))
