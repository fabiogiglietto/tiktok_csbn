...
generate_network_visualization <- function(coordination_results, 
  ...
  if (!requireNamespace("htmltools", quietly = TRUE)) {
    stop("Package 'htmltools' is required. Please install it using install.packages('htmltools')")
  }
  ...
  if (!dir.exists(viz_dir)) {
    ...
    if (!dir.exists(viz_dir)) {
      stop("Failed to create visualization directory")
    }
  }
  
  # Extract graph and ensure it's valid
  ...
  message(sprintf("Number of vertices: %d", igraph::vcount(graph)))
  message(sprintf("Number of edges: %d", igraph::ecount(graph)))
  message("Graph attributes:")
  print(igraph::graph_attr_names(graph))
  message("Vertex attributes:")
  print(igraph::vertex_attr_names(graph))
  message("Edge attributes:")
  print(igraph::edge_attr_names(graph))
  
  # Process node attributes
  node_data <- data.frame(
    id = igraph::V(graph)$name,
    url = paste0("https://tiktok.com/@", igraph::V(graph)$name),
    degree = igraph::degree(graph),
    component = igraph::components(graph)$membership,
    stringsAsFactors = FALSE
  )
  
  # Create informative tooltips
  node_data$title <- sprintf(
    paste0(
      "<div class='node-tooltip'>",
      "<p><strong>Account:</strong> @%s</p>",
      "<p><strong>Connections:</strong> %d</p>",
      "<p><strong>Network Group:</strong> %d</p>",
      "</div>"
    ),
    node_data$id,
    node_data$degree,
  ...
  # Convert to visNetwork format with error handling
  tryCatch({
    network_data <- visNetwork::toVisNetworkData(graph)
    message("Network data after conversion:")
    message(sprintf("Number of nodes: %d", nrow(network_data$nodes)))
    message(sprintf("Number of edges: %d", nrow(network_data$edges)))
    message("Node columns:")
    print(names(network_data$nodes))
    message("Edge columns:")
    print(names(network_data$edges))
  }, error = function(e) {
    stop(paste("Failed to convert graph to visNetwork format:", e$message))
  })
  
  # Enhance nodes with size and component information
  network_data$nodes <- network_data$nodes %>%
    dplyr::left_join(node_data, by = "id") %>%
    dplyr::mutate(
      size = degree * node_size_multiplier + node_size_base,
      shape = "dot",
      shadow = TRUE
    )
  
  # Add cluster labels if available
  if (exists("cluster_labels") && is.data.frame(get("cluster_labels"))) {
    label_data <- get("cluster_labels") %>%
      dplyr::mutate(
        component = as.numeric(component),
        group_label = sprintf("Network %d - %s", component, label)
      )
    
    network_data$nodes <- network_data$nodes %>%
      dplyr::left_join(
        label_data %>% dplyr::select(component, group_label),
        by = "component"
      ) %>%
      dplyr::mutate(
    ...
    network_data$nodes$group <- sprintf("Network %d", network_data$nodes$component)
  }
  
  # Process edges with weights
  network_data$edges <- network_data$edges %>%
    dplyr::mutate(
      width = if ("weight" %in% names(.)) {
        weight * edge_width_multiplier
      } else {
        rep(1, nrow(.))
      },
      smooth = TRUE,
      arrows = "to"  # Add arrows for directed graphs if needed
    ) %>%
    mutate(
      # Set edge colors - each edge gets its own color object
      color = lapply(seq_len(nrow(.)), function(i) {
        list(
          color = "#666666",
          highlight = "#3388ff"
        )
  ...
  custom_css <- paste0(
    "<style>",
    ".vis-network { border: 1px solid #ddd; border-radius: 4px; min-height: 600px; }",
    ".node-tooltip { padding: 10px; max-width: 300px; background: white; border: 1px solid #ddd; border-radius: 4px; }",
    ".node-tooltip p { margin: 5px 0; }",
    ".vis-network-configuration { padding: 10px; background: #f8f9fa; border-radius: 4px; }",
    ".vis-network-configuration select { max-width: 100%; padding: 5px; }",
    "</style>"
  )
  
  # Create network visualization with enhanced configuration
  ...
  network <- htmlwidgets::onRender(
    network,
    sprintf("function(el, x) { el.innerHTML += '%s'; }", custom_css)
  )
  ...
  tryCatch({
    ...
    message(sprintf("Network visualization saved to: %s", html_file))
  }, error = function(e) {
    warning(paste("Failed to save network visualization:", e$message))
  })
  ...
  message(sprintf(
    ...
    igraph::gorder(graph),
    igraph::gsize(graph),
    max(igraph::components(graph)$membership)
  ))
  
  # Final validation before returning
  if (is.null(network)) {
    stop("Network object is NULL")
  }
  
  message("Network object created successfully")
  message("Network configuration:")
  print(network$x$options)
  
  return(network)
}
```

```{r generate_visualization, results='asis'}
