...
```{r fetch_all_videos, message=FALSE, eval=!params$load_from_snapshot, cache=TRUE}
...
log_data <- read_csv("/home/mine/VERAAI_WP4/output/new_accounts_log.csv", show_col_types = FALSE) %>%
  # Remove future dates
  filter(Timestamp <= Sys.Date()) %>%
  # Filter out unrealistic values (e.g., more than 1000 new accounts per day)
  filter(New_Accounts_Count <= 1000) %>%
  # Aggregate by date
  group_by(Timestamp) %>%
  summarise(Daily_Total = sum(New_Accounts_Count))

# Create bar plot with clean data
ggplot(log_data, aes(x = Timestamp, y = Daily_Total)) +
